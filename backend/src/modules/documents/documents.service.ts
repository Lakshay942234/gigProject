...
export class DocumentsService {
  ...
  constructor(
    ...
    this.bucketName =
      this.configService.get<string>('MINIO_BUCKET') || 'bpo-documents';
  ...
  async uploadDocument(candidateId: string, file: any, type: DocumentType) {
    if (!file) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to upload document for candidate ${candidateId}`,
        error,
      );
