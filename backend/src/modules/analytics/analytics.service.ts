...
export class AnalyticsService {
    ...
    async createDailySnapshot() {
        ...
        const qualifiedAgents = await this.prisma.candidate.count({ where: { qualifiedToWork: true } });

        // Active agents (had a session today)
        const activeAgents = await this.prisma.workSession.groupBy({
            by: ['candidateId'],
            where: { startTime: { gte: today } },
        }).then(res => res.length);

        const totalSessions = await this.prisma.workSession.count({ where: { startTime: { gte: today } } });
        const totalChats = await this.prisma.chatSession.count({ where: { startTime: { gte: today } } });
