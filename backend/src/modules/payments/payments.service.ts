...
export class PaymentsService {
  ...
  async getAllPayouts(status?: PayoutStatus) {
    return this.prisma.payout.findMany({
      where: status ? { status } : {},
      include: {
        wallet: { include: { candidate: { include: { user: true } } } },
      },
  ...
  async updatePayoutStatus(id: string, updateDto: UpdatePayoutStatusDto) {
    ...
    return this.prisma.payout.update({
      ...
      data: {
        ...
        processedAt:
          updateDto.status !== PayoutStatus.PENDING ? new Date() : undefined,
        completedAt:
          updateDto.status === PayoutStatus.COMPLETED ? new Date() : undefined,
      },
    });
  }
}
